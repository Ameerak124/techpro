...
$stmt2=$pdoread->prepare("SELECT `sno`, `transid`, `formid`, `questionid`, `question_name`, `question_type`, `answers`, Date_format(`createdon`,'%d-%b-%Y %H:%i %p') As date, `createdby`, `status` FROM `dynamic_form_submit_answer` where `formid`=:form_id AND transid=:transid and (questionid LIKE :searchterm || question_name LIKE :searchterm)");
