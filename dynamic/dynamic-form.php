...
$check = $pdoread -> prepare("SELECT `sno`, `form_id`, `formname`, Concat(`formdescription` ,'( ',`department`,' )') AS formdescription, `usage_type`, `frequency`, `freqcount`, `url_name`, `createdon`, `createdby`, `modifiedon`, `modifiedby`, `estatus`, `costcenter`,`signature` FROM `main_dynamic_form` WHERE `form_id`=:form_id AND `estatus`='Active'");
...
if($stmt2 -> rowCount() > 0){
      ...
      "access"=>false
		...
		array_push($response1,$temptime);	 
			  }else if($row['question_type']=="Linear scale"){
          $temptime=[
       "type"=>"seekbar",
       "required"=>false,
       "label"=>$row['question_name'],
