...
if($check -> rowCount() > 0){
// Updated details
$saleprice = $pdo4 -> prepare("UPDATE `billing_history` SET `modifiedby`= :userid,`modifiedon`=CURRENT_TIMESTAMP,`ipaddress`= :ipaddress,`quantity`= :qty,`total` = ROUND(:qty*`rate`,2) WHERE `sno` = :track AND `status` = 'Hold'");
