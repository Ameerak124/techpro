...
$response = array();
try{
if(!empty($accesskey) &&!empty($fdate) &&!empty($todate) &&!empty($status)){
$check = $pdoread -> prepare("SELECT `userid` FROM `user_logins` WHERE `accesskey` = :accesskey");
$check->bindParam(':accesskey', $accesskey, PDO::PARAM_STR);
$check -> execute();
if($check -> rowCount() > 0){
$emp = $check -> fetch(PDO::FETCH_ASSOC);
$stmt1 = $pdoread->prepare("SELECT `po_generate`. `po_number` , `vendormaster`.`legalname`,`po_generate`.`po_total`,`unit_master`.`unit`,`po_generate`.`po_status` FROM `po_generate`INNER JOIN `vendormaster` ON `po_generate`.`vendor_sno` = `vendormaster`.`sno` INNER JOIN `unit_master` ON `po_generate`.`invoice_unit`= `unit_master`.`sno` WHERE (date(`created_on`) BETWEEN :fdate AND :todate ) AND `po_status` =:status");
...
$stmt1 -> execute(); 
   $ponum_list = $stmt1->fetchAll(PDO::FETCH_ASSOC);
     if($stmt1 -> rowCount() > 0){
     http_response_code(200);
          $response['error']= false;
	     $response['message']="Data found";
		 $response['polist']= $ponum_list;
     }
	 else
     {
