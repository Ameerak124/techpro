   ...
   $textview= [
	...
	array_push($response1,$textview);		 
	  ...
	  $response['message'] = $response1;
	  	...
	  	$stmt3=$con->prepare("SELECT `patient_sign_one`,`witness_sign`,`doctor_sign`,`interpreter_sign` FROM `consent_patient_data` where `status`='Active' and consent_id='CONS00003' and `ipno`='INV55240100453'");
	$stmt3 -> execute();
	  $row3 = $stmt3->fetch(PDO::FETCH_ASSOC);
	   $response['patientsign'] = $row3['patient_sign_one'];
	   $response['wittnesssign'] = $row3['witness_sign'];
	   $response['doctorsign'] = $row3['doctor_sign'];
	   $response['interpretersign'] = $row3['interpreter_sign'];
     }
	 else
     {
