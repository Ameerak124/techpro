 ...
 if($result['role'] == 'AUDIT'){
   ...
   if($insert_data->rowCount() > 0){
    ...
    $response['message']="No Data Found";
	  ...
	  $insert_data=$pdoread->prepare("SELECT @a:=@a+1 AS sno,`op_billing_generate`.`patient_name`, `refund_id`, `op_refund_generate`.`invoice_no`, `op_refund_generate`.`umr_no`, `op_refund_generate`.`billno`,  `op_refund_generate`.`original_value` as gross, `item_wise_discount` as discount,   `total` as 'bill_amt', `op_billing_generate`.`organization_code`,`op_billing_generate`.`organization_name`,DATE_FORMAT(`op_refund_generate`.`created_on`,'%d-%b-%Y') AS Bill_date, DATE_FORMAT(`op_refund_generate`.`created_on`,'%h:%i %p') AS Bill_time,if(`stage2_status`='','Pending',stage2_status) as stage1_status,IF(stage1_status='Approved' and stage2_status='','Yes','No') as 'Approve_button',IF(stage1_status='Pending','No','No') as 'Reject_button',`stage2_by` as stage1_by,IF(stage1_status='Approved' and stage2_status='Approved' and stage3_status='Approved',concat('https://techpro.medicoveronline.com/refund-bill-pdf.php?a=',:accesskey,'&no=',refund_id),'') AS report FROM (SELECT @a:=0) AS a,`op_refund_generate` left join `op_billing_generate` ON `op_billing_generate`.`invoice_no`=`op_refund_generate`.`invoice_no` WHERE  `bill_status`='Approved' AND `refund_status` IN ('Raised', 'Confirmed')  AND `op_refund_generate`.`cost_center`=:branch  AND  date(op_refund_generate.`created_on`) BETWEEN :fromdate AND :todate ");
   $insert_data->bindParam(':branch',$result['cost_center'],PDO::PARAM_STR);
   $insert_data->bindParam(':fromdate',$fromdate,PDO::PARAM_STR);
   $insert_data->bindParam(':todate',$todate,PDO::PARAM_STR);
    $insert_data->bindParam(':accesskey',$accesskey,PDO::PARAM_STR);
   ...
   if($insert_data->rowCount() > 0){
    ...
    $response['message']="No Data Found";
	  ...
	  $insert_data=$pdoread->prepare("SELECT @a:=@a+1 AS sno,`op_billing_generate`.`patient_name`, `refund_id`, `op_refund_generate`.`invoice_no`, `op_refund_generate`.`umr_no`, `op_refund_generate`.`billno`,  `op_refund_generate`.`original_value` as gross, `item_wise_discount` as discount,   `total` as 'bill_amt', `op_billing_generate`.`organization_code`,`op_billing_generate`.`organization_name`,DATE_FORMAT(`op_refund_generate`.`created_on`,'%d-%b-%Y') AS Bill_date, DATE_FORMAT(`op_refund_generate`.`created_on`,'%h:%i %p') AS Bill_time,if(`stage3_status`='','Pending',stage3_status) as stage1_status,IF(stage1_status='Approved' and stage2_status='Approved' and  stage3_status='','Yes','No') as 'Approve_button',IF(stage1_status='Pending','No','No') as 'Reject_button',`stage3_by` as stage1_by,IF(stage1_status='Approved' and stage2_status='Approved' and stage3_status='Approved',concat('https://techpro.medicoveronline.com/refund-bill-pdf.php?a=',:accesskey,'&no=',refund_id),'') AS report FROM (SELECT @a:=0) AS a,`op_refund_generate` left join `op_billing_generate` ON `op_billing_generate`.`invoice_no`=`op_refund_generate`.`invoice_no` WHERE  `bill_status`='Approved' AND `refund_status` IN ('Raised', 'Confirmed')  AND `op_refund_generate`.`cost_center`=:branch  AND  date(op_refund_generate.`created_on`) BETWEEN :fromdate AND :todate ");
   $insert_data->bindParam(':branch',$result['cost_center'],PDO::PARAM_STR);
   $insert_data->bindParam(':fromdate',$fromdate,PDO::PARAM_STR);
   $insert_data->bindParam(':todate',$todate,PDO::PARAM_STR);
    $insert_data->bindParam(':accesskey',$accesskey,PDO::PARAM_STR);
