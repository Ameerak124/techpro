...
(SELECT CONCAT(`doctor_master`.`title`,'',`doctor_master`.`doctor_name`)AS docname FROM `doctor_master` WHERE `doctor_master`.`doctor_uid`=`doctor_assessment`.`doctor_uid`)AS doctor_name
FROM `doctor_assessment` WHERE `status`='Active' AND  `umr_no`=:umrno AND `$category` NOT IN('') GROUP BY catageory");
