...
try {
     if(!empty($accesskey) && !empty($sno) && !empty($del_remarks)){    
...
$update=$pdo4->prepare("UPDATE `predicting_pressure_score` SET `estatus`='Inactive' , `modifiedby`=:userid , `modifiedon`=CURRENT_TIMESTAMP,`del_remarks`=:del_remarks WHERE `sno`=:sno AND `cost_center`=:cost_center");
