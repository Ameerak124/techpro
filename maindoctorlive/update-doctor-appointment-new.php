...
if(!empty($slottime) && !empty($doctorid) && !empty($selectdate) && !empty($accesskey) && !empty($patientname) && !empty($mobile) && !empty($gender) && !empty($transid) && !empty($reqno)&& !empty($invoice_no)&& !empty($umr) && !empty($branch) && (!empty($consultation_type))){
...
if(($selectdate > $current_date) || ($selectdate == $current_date && $slottime > $current_slot)){
	...
	,`requisition_no`=:reqno,`invoice_no`=:invoice_no , `umrno`=:umr , `visit_count`=:visit_count ,`patient_lead_source`=:patient_lead_source ,`modifiedon` =CURRENT_TIMESTAMP,consultation_type=:consultation_type
	WHERE (`transid` = :transid AND `slot_status` ='Hold' AND `location`=:branch)OR (`transid` = :transid AND `slot_status` ='booked' AND `location`=:branch)");
				  ...
				  $check2->bindParam(':consultation_type', $consultation_type, PDO::PARAM_STR);
