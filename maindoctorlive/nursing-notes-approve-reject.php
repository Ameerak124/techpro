...
if($check -> rowCount() > 0){
    ...
    $update = $pdo4->prepare("UPDATE `nursing-notes-tbl` SET `approved_status`= :approved_status, `approvedby`= :userid, `approvedon`=CURRENT_TIMESTAMP WHERE `admissionno` = :admissionno AND `umr_no`=:umrno AND `sno` =:sno AND `estatus`='Active' ");
