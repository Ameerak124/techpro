...
$sedation=trim($data->sedation);
$sedation_score=trim($data->sedation_score);
$remarks=trim($data->remarks);  
try {
    if(!empty($accesskey) && !empty($ipno)&& !empty($umrno) && ($sedation_score) >= 0 && ($sedation_score) !=''){  
...
//insert query
$ninsert=$pdo4->prepare("INSERT IGNORE INTO `sedation_score`(`sno`, `createdfrom`,`ipno`, `umrno`,  `sedation`, `sedation_score`, `remarks`, `estatus`, `createdon`, `createdby`, `modifiedon`, `modifiedby`, `cost_center`,`del_remarks`) VALUES (NULL,'Score Assessment',:ipno,:umrno, :sedation, :sedation_score,:remarks, 'Active',CURRENT_TIMESTAMP,:userid,CURRENT_TIMESTAMP,:userid,:costcenter,'')");
$ninsert->bindParam(':ipno', $ipno, PDO::PARAM_STR);
$ninsert->bindParam(':umrno', $umrno, PDO::PARAM_STR);
$ninsert->bindParam(':sedation', $sedation, PDO::PARAM_STR);
$ninsert->bindParam(':sedation_score', $sedation_score, PDO::PARAM_STR);
