...
try{
     if(!empty($accesskey)){
          $accesscheck ="SELECT `userid`,`cost_center` FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status` = 'Active'";
               ...
               $my_array = array("Oral","IV(intraVenus)","IM(intraMascular)","Sub-Cutaneous(SC/SQ)","Sub-Lingual","Perenteral","Perrectral","Patch","Intradermal","Nasal","Occural/Eye","Otic/Ear");
              http_response_code(200);
              $response['error'] = false; 
              $response['message']= "Data Found";
              for($x = 0; $x < sizeof($my_array); $x++){
                    $response['Opdroutelist'][$x]['routename']=$my_array[$x];	
               }	
                   }else{
