...
foreach($mmss as $keys) { 
    ...
    $percheck1 ="SELECT `submenu` As name,`submenu` As displayname,`submenu` As displaynames,`titlename` As title,storyboardname,`packagename` As packagename, `classname`, `classtype`,ioscontrollers FROM `android_permissions_new` WHERE `sno`=:key AND `menu`=:menu AND `status`=:status and source = 'Doctor'";
