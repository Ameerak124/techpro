...
if($check -> rowCount() > 0){
     ...
     $empname = $empdata['userid'];
      $stmt = $pdoread -> prepare("SELECT IFNULL(MAX(`consumption_id`),'CNS00000') AS `consumptionid` FROM `ms_billing`.`generate_consumption` LIMIT 1");
      $stmt -> execute(); 
     if($stmt -> rowCount() > 0){
		...
		http_response_code(200);
	     ...
	     $response['message']="Data Found";
          $response['consumptionid']= ++$consumptionid;
     }
	 else
     {
