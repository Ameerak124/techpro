...
$accesscheck ="SELECT `userid`,`branch`,`cost_center`,username,role FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status` = 'Active'";
	...
	$result2 = $pdo1 -> prepare("SELECT `sno`, `id`,`name` FROM `visitor_pass` WHERE `sno`=:sno");
	$result2->bindParam(':sno', $sno, PDO::PARAM_STR);
	$result2-> execute();
    $data = $result2->fetch(PDO::FETCH_ASSOC);
	
    $result = $pdo1 -> prepare("UPDATE `visitor_pass` SET `modified_by`=:userid,`modified_on`=CURRENT_TIMESTAMP,`out_time`=CURRENT_TIME,status='Exit' WHERE sno=:sno and status='Entered'");
	...
	$result-> execute();
	 ...
	 $result1 = $pdo1 -> prepare("INSERT INTO `visitor_pass_logs`(`id`, `name`,`time`, `created_by`,`created_on`,`status`) VALUES (:id,:name,CURRENT_TIME,:userid,CURRENT_TIMESTAMP,'Exit')");
	$result1->bindParam(':id', $data['id'], PDO::PARAM_STR);
	$result1->bindParam(':name', $data['name'], PDO::PARAM_STR);
