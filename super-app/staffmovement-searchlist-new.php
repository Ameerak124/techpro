...
$query1=$pdo1->prepare("SELECT `Sno`, `id` AS staffmovementid,`emp_id`, `emp_name`, `desgination`, `branch`, `reason`, `reason_remarks`, `created_by`, DATE_FORMAT(`created_on`,'%d-%b-%Y %H:%i:%s') as created_on, `modified_by`, IFNULL(DATE_FORMAT(`modified_on`,'%d-%b-%Y %H:%i:%s'),'') as `modified_on`, `in_time`, `out_time`,status FROM `staff_movement` where (emp_id like :searchterm  || emp_name like :searchterm) and date(created_on) BETWEEN :fdate and :tdate and branch=:branch order by Sno desc");
