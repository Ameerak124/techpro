...
$accesscheck ="SELECT `userid`,`branch`,`cost_center`,username,role FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status` = 'Active'";
	...
	$my_array = array("Abas","Venkatesh","Muna","Suresh");
     }else if($row['cost_center']=='MWCHYD'){
		   $my_array = array("Chandra Kumar","S.Srikanth","N.Vishnu Vardhan","Anjaneyulu","Ajay");
     }
    ...
    for($x = 0; $x < sizeof($my_array); $x++){
		$response['driverlist'][$x]['drivername']=$my_array[$x];	
     }	
    	}else{
