...
if(($type=='E Assist') || ($type=='Security Dashboard') || ($type=='Dynamic Form') || ($type=='Permissions') || ($type=='Get Data') || ($type=='Journal') || ($type=='Tech Support') || ($type=='PACS') || ($type=='Appointment Slot Summary')){
...
}else if(($type=='International Operations')){
	$fetch = $pdoread -> prepare("SELECT `userid`,`username`,`mobile_accesskey`,`branch` AS storeaccess ,cost_center,role FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status`= 'Active'");
	$fetch->bindParam(':accesskey', $accesskey, PDO::PARAM_STR);
	//$fetch->bindParam(':password', $password, PDO::PARAM_STR);
	$fetch -> execute();
	...
	$fetch = $pdoread -> prepare("SELECT `userid`,`username`,`mobile_accesskey`,`branch` AS storeaccess ,cost_center,role FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status`= 'Active' and role in ('Doctor','Center Head','Medical Head','Director Operations','ED','Corporate Head')");
	$fetch->bindParam(':accesskey', $accesskey, PDO::PARAM_STR);
	//$fetch->bindParam(':password', $password, PDO::PARAM_STR);
	$fetch -> execute();
	if($fetch->rowCount()>0){
...
}else if($type=='Doctor' || $type=='E-MIS' || $type=='Operations' ||$type=='My Article' ||$type=='Clinical News'){
