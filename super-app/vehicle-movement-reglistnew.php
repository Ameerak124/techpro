...
$query1=$pdo1->prepare("SELECT `sno`, `id`, `vehicle_no`, `driver_name`, if (`in_time`='00:00:00', '', in_time) as in_time,if (`out_time`='00:00:00', '', out_time) AS out_time, `reading_in`, `reading_out`,(`reading_in`-`reading_out`)  as totaldiff,`aurthrized_by`, `place`, `purpose`, `remarks`, `created_by`, DATE_FORMAT(`created_on`,'%d-%b-%Y %H:%i:%s') as created_on, `modified_by`, IFNULL(DATE_FORMAT(`modified_on`,'%d-%b-%Y %H:%i:%s'),'') as `modified_on`, `status`,if (`status`='Entered', '#4CAF50', '#FF9800') as status_colour FROM `vehicle_movement_reg` where date(created_on) BETWEEN :fdate and :tdate   order by sno desc ");
