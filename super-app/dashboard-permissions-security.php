...
$accesscheck ="SELECT `userid`,`branch`,`cost_center`,username,role,androidpermissions,androidsubmenu,androiddashboard FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status` = 'Active'";
	...
	//echo $row['androiddashboard'];
 foreach($mmsss as $key) { 
$percheck ="SELECT `menu`,`submenu` As name,`dashboardtitle` As title,`packagename` As packagename,`ioscontrollers` as webcontroller,`classname`, `classtype`,CONCAT(:baseurl,'appicons/',`d-icons`) AS icon,storyboardname FROM `android_permissions_new` WHERE `sno`=:key  and source='Security'";
