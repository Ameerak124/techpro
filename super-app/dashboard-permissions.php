...
$percheck ="SELECT `menu`,`submenu` As name,`dashboardtitle` As title,`packagename` As packagename,`ioscontrollers` as webcontroller,`classname`, `classtype`,CONCAT(:baseurl,'appicons/',`d-icons`) AS icon,storyboardname,`d_url`,`d_url1` FROM `android_permissions_new` WHERE `sno`=:key and source='superapp' and status='Active'";
