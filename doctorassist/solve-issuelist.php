...
if(!empty($accesskey)){
   ...
   /*  $displaybutton ="SELECT  if(`assign`='1','Assign','No') AS assign_button, if(`escalate`='1','Escalate','No') AS `escalate_button`,if(`reject`='1','Reject','No') AS`reject`, if(`resolve`='1','Resolve','No') AS `resolve`,if(`inprocessing`='1','Inprocessing','No') as inprocessing,if(`assign`='1','#E7FDEA','No') AS bgassign, if(`escalate`='1','#E5E9FF','No') AS `bgescalate`,if(`reject`='1','#FFD9D9','No') AS`bgreject`, if(`resolve`='1','#BBF4DB','No') AS `bgresolve`,if(`inprocessing`='1','#E9bbF4','No') as bgprocessing,if(`assign`='1','#49B888','No') AS textassign, if(`escalate`='1','#369DB3','No') AS `textescalate`,if(`reject`='1','#FD1F0F','No') AS`textreject`, if(`resolve`='1','#407529','No') AS `textresolve`,if(`inprocessing`='1','#5E2975','No') as textprocessing FROM `issue_level_designations` WHERE FIND_IN_SET(:role,`CL`)<>0 LIMIT 1";
	...
	$stmt1->bindValue(":baseurl", $baseurl, PDO::PARAM_STR);
   ...
   $stmt1->execute();
    ...
    $rows = $stmt12->fetch();	
		...
		//$button1=$rows['assign_button'];
				...
				$stmt1 = $pdoread->prepare("SELECT doctor_raise_ticket.`sno`, doctor_raise_ticket.`type`, doctor_raise_ticket.`ticket_id`, doctor_raise_ticket.`category`, doctor_raise_ticket.`issues`,if(doctor_raise_ticket.`issues`='Any other Suggestions',doctor_raise_ticket.`issue_details`,'') as issue_remarks, doctor_raise_ticket.`issue_details`,doctor_raise_ticket.`remarks`,doctor_raise_ticket.`photo`, case when doctor_raise_ticket.`status`='Pending' then concat(:baseurl,'mobile/appicons/pending.png') else concat(:baseurl,'appicons/solve.png') end  as icon , CASE WHEN doctor_raise_ticket.`status`='Pending' THEN '#F1732E' ELSE '#419505' END AS color ,issue_logs.`created_by`, date_format(doctor_raise_ticket.`created_on`,'%d-%b-%Y %h:%i:%s') as created_on , doctor_raise_ticket.`modified_by`, doctor_raise_ticket.`modified_on`, if(doctor_raise_ticket.`photo`='Please select an image file to upload.' OR doctor_raise_ticket.`photo`='Yes' or doctor_raise_ticket.`photo`='Invalid image file'  or doctor_raise_ticket.`photo`='','',concat(:baseurl,'mobile/',doctor_raise_ticket.`photo`)) as image ,issue_logs.`status`,doctor_raise_ticket.assigned_person,branch,branch_master.display_name as branchname FROM `doctor_raise_ticket` inner join issue_logs ON issue_logs.issue_sno=doctor_raise_ticket.sno inner join branch_master on doctor_raise_ticket.branch=branch_master.cost_center WHERE  issue_logs.created_by like :empid  and issue_logs.`status` like (:status '' :role)  AND date(doctor_raise_ticket.created_on) between :fdate and :tdate group by issue_logs.issue_sno order by issue_logs.`sno` DESC");
	$stmt1->bindValue(":empid", "%{$row['employee_id']}%", PDO::PARAM_STR);
		$stmt1->bindParam(':fdate', $fdate, PDO::PARAM_STR);
	$stmt1->bindParam(':tdate', $tdate, PDO::PARAM_STR);
	$stmt1->bindValue(":status", "%{$status}%", PDO::PARAM_STR);
	$stmt1->bindValue(":role", "%{$row['role']}%", PDO::PARAM_STR);
    ...
    $rows = $stmt12->fetch();	
		...
		//$button1=$rows['assign_button'];
				...
				$stmt1 = $pdoread->prepare("SELECT doctor_raise_ticket.`sno`, doctor_raise_ticket.`type`, doctor_raise_ticket.`ticket_id`, doctor_raise_ticket.`category`, doctor_raise_ticket.`issues`,if(doctor_raise_ticket.`issues`='Any other Suggestions',doctor_raise_ticket.`issue_details`,'') as issue_remarks, doctor_raise_ticket.`issue_details`,doctor_raise_ticket.`remarks`,doctor_raise_ticket.`photo`, case when doctor_raise_ticket.`status`='Pending' then concat(:baseurl,'mobile/appicons/pending.png') else concat(:baseurl,'appicons/solve.png') end  as icon , CASE WHEN doctor_raise_ticket.`status`='Pending' THEN '#F1732E' ELSE '#419505' END AS color ,issue_logs.`created_by`, date_format(doctor_raise_ticket.`created_on`,'%d-%b-%Y %h:%i:%s') as created_on , doctor_raise_ticket.`modified_by`, doctor_raise_ticket.`modified_on`, if(doctor_raise_ticket.`photo`='Please select an image file to upload.' OR doctor_raise_ticket.`photo`='Yes' or doctor_raise_ticket.`photo`='Invalid image file'  or doctor_raise_ticket.`photo`='','',concat(:baseurl,'mobile/',doctor_raise_ticket.`photo`)) as image ,doctor_raise_ticket.status,doctor_raise_ticket.assigned_person ,branch,branch_master.display_name as branchname FROM `doctor_raise_ticket` inner join issue_logs ON issue_logs.issue_sno=doctor_raise_ticket.sno  inner join branch_master on doctor_raise_ticket.branch=branch_master.cost_center WHERE  issue_logs.created_by like :empid and doctor_raise_ticket.`status` like (:status '' :role) and doctor_raise_ticket.status!='Pending'  AND date(doctor_raise_ticket.created_on) between :fdate and :tdate GROUP BY doctor_raise_ticket.`sno` order by doctor_raise_ticket.`sno` DESC");
	$stmt1->bindValue(":empid", "%{$row['employee_id']}%", PDO::PARAM_STR);
	$stmt1->bindValue(":status", "%{$status}%", PDO::PARAM_STR);
	$stmt1->bindValue(":role", "%{$row['role']}%", PDO::PARAM_STR);
	$stmt1->bindValue(":baseurl", $baseurl, PDO::PARAM_STR);
...
/*   $response['btn1']=$row1['assign_button'];
  $response['btn2']=$row1['escalate_button'];
  $response['btn3']=$row1['resolve'];
  $response['btn4']=$row1['reject'];
  $response['displaybtn1']=$row1['assign_button'];
  $response['displaybtn2']=$row1['escalate_button'];
  $response['displaybtn3']=$row1['resolve'];
  $response['displaybtn4']=$row1['reject']; */
