...
if($row['role']=='Center Head' || $row['role']=='Medical Head'){
  ...
  $stmt11=$pdoread->prepare("SELECT `ticket_id` FROM `doctor_raise_ticket` WHERE `sno`=:sno");
   ...
   $rows = $stmt11->fetch(PDO::FETCH_ASSOC);
	...
	$stmt1=$pdo4->prepare("UPDATE `doctor_raise_ticket` SET assigned_person=:assigning_person,`remarks`=:remarks,realstatus=:statusact,category_type=:categorytype WHERE `sno`=:sno");
	...
	$stmt2=$pdo4->prepare("INSERT INTO `issue_logs`( `issue_sno`, `assigned_person`,role, `created_by`, `created_on`, `reason`, `status`,created_by_name,ticket_id,`category_type`) VALUES(:sno,:assigning_person,:designation,:empidd,CURRENT_TIMESTAMP,:remarks,:status,:name,:ticket_id,:categorytype)");
