...
$count = $pdoread -> prepare("SELECT COALESCE((SELECT COUNT(*) FROM `doctor_raise_ticket` WHERE DATE(doctor_raise_ticket.created_on) between :fdate and :tdate and user_id=:empid),0) as Raised,(COALESCE((SELECT COUNT(*) FROM `doctor_raise_ticket` WHERE status NOT LIKE '%Resolve%' and status NOT LIKE '%Reject%' AND DATE(doctor_raise_ticket.created_on) between :fdate and :tdate and user_id=:empid ),0)) as opened,(COALESCE((SELECT COUNT(*) FROM `doctor_raise_ticket` WHERE (status  LIKE '%Resolve%' or status LIKE '%Reject%') AND DATE(doctor_raise_ticket.created_on) between :fdate and :tdate and user_id=:empid),0)) as Closed");
