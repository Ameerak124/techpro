...
if($check -> rowCount() > 0){
	...
	"unit"=> "Unit",
	"unitcode"=> "",
	"unitheadcount"=>"Unit Head",
	"unitheading"=>"Pending at Unit Head",
	"corporateheadcount"=>"Corporate Head",
	...
	$revenue_list = $pdoread -> prepare("SELECT COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE `realstatus`in ('Pending at Center Head','Pending by Sub Center head') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS unitheadpending,COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE `realstatus`in ('Pending at Medical Head') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS medicalheadpending,COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE `realstatus`IN ('Pending by Corporate Head','Pending by Sub Corporate Head') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS pencorhead,COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE `realstatus`in ('Pending by Director Operations','Pending by Sub Director Operations') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS pendirector,COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE `realstatus`IN ('Pending by ED','Pending by Sub ED') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS pendinged,COALESCE((SELECT count(*) FROM `doctor_raise_ticket` WHERE (`realstatus` LIKE '%Resolve%' or `realstatus` LIKE  '%Reject%') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate and category_type=:category),0) AS closedged");
		$revenue_list->bindParam(':fdate', $fdate, PDO::PARAM_STR);
	$revenue_list->bindParam(':tdate', $tdate, PDO::PARAM_STR);
	$revenue_list->bindParam(':branch', $result1['cost_center'], PDO::PARAM_STR);
   ...
   $response['title']="Trend Analysis";
	...
	$unittotal=$unittotal+$unitheadcount;
