...
if($row['role']=='Center Head' || $row['role']=='Medical Head'){
...
$stmt22= $pdoread->prepare("SELECT `assigned_person`,created_by  FROM `issue_logs` WHERE `issue_sno`= :sno and `status`like '%Pending%'");
	   ...
	   $type = $status .  ' by ' .$role;
		...
		if($status=="Escalated"){
	$stmt1=$pdo4->prepare("UPDATE `doctor_raise_ticket` SET assigned_person=:assignperson,category_type=:categorytype WHERE `sno`=:sno");

	$stmt1->bindParam(":assignperson", $rows['assigned_person'], PDO::PARAM_STR);
	$stmt1->bindParam(":categorytype", $categorytype, PDO::PARAM_STR);
	...
	$stmt21=$pdo4->prepare("INSERT INTO `issue_logs`(`issue_sno`, `ticket_id`, `assigned_person`, `role`, `created_by`, `created_by_name`, `created_on`, `reason`, `status`,`category_type`)VALUES (:sno,:ticket_id,:assignperson,:role,:empidd,:name,CURRENT_TIMESTAMP,:remarks,:status,:categorytype)");
	$stmt21->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt21->bindParam(":categorytype", $categorytype, PDO::PARAM_STR);
	...
	$stmt21->execute();
			...
			$stmt1=$pdo4->prepare("UPDATE `doctor_raise_ticket` SET assigned_person=:assignperson,status=:status,realstatus=:type,category_type=:categorytype WHERE `sno`=:sno");
...
$stmt22= $pdoread->prepare("SELECT `assigned_person`,created_by  FROM `issue_logs` WHERE `issue_sno`= :sno and `status`like '%Pending%'");
	   ...
	   $type = $status .  ' by ' .$role;
		...
		if($status=="Escalated"){
	$stmt1=$pdo4->prepare("UPDATE `doctor_raise_ticket` SET assigned_person=:assignperson WHERE `sno`=:sno");
	...
	$stmt21=$pdo4->prepare("INSERT INTO `issue_logs`(  `issue_sno`, `ticket_id`, `assigned_person`, `role`, `created_by`, `created_by_name`, `created_on`, `reason`, `status`)VALUES (:sno,:ticket_id,:assignperson,:role,:empidd,:name,CURRENT_TIMESTAMP,:remarks,:status)");
	$stmt21->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt21->bindParam(":assignperson", $rows['assigned_person'], PDO::PARAM_STR);
	$stmt21->bindParam(":role", $row['role'], PDO::PARAM_STR);
	$stmt21->bindParam(":ticket_id", $row22['ticket_id'], PDO::PARAM_STR);
	...
	$stmt21->execute();
			...
			$stmt1=$pdo4->prepare("UPDATE `doctor_raise_ticket` SET assigned_person=:assignperson,status=:status,realstatus=:type WHERE `sno`=:sno");
	//$stmt1->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt1->bindParam(":assignperson", $row['assigningperson'], PDO::PARAM_STR);
	//$stmt1 -> bindParam(":ImagePath", $ImagePath, PDO::PARAM_STR);
	$stmt1 -> bindParam(":status", $type, PDO::PARAM_STR);
