...
try {
if(!empty($accesskey) && !empty($track)){
//Check access 
...
if($check -> rowCount() > 0){
// Updated details
$saleprice = $pdo4 -> prepare("UPDATE `billing_history` SET `modifiedby`=:userid,`modifiedon`=CURRENT_TIMESTAMP,`ipaddress`= :ipaddress,`status`='Hidden' WHERE `sno` = :track AND `status` = 'Hold'");
