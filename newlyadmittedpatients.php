...
if($check -> rowCount() > 0){
	if($searchname == ''){
      ...
      $adm_pnts =$pdoread->prepare("SELECT `patientname`,`admissionno`,datediff(CURRENT_DATE,`admittedon`) as lossofdays,`admittedward` as wardname,`consultantname`,Date_format(`admittedon`,'%d-%b-%Y %H:%S') As admissiondate,'Cash' AS patientype,'CAS' As ptypeshort,`contactno`,Concat(`admissionno`,'/',Date_format(`admittedon`,'%d-%b-%Y')) As ipdate,'' AS companyname, (DATE_FORMAT(FROM_DAYS(DATEDIFF(now(),`patientage`)), '%Y')+0) AS `patientage`,`patientgender`,Date_format(`admittedon`,'%d-%b-%Y') As`admittedon`,`contactno` FROM `registration` WHERE `consultantname` = :consultantname ORDER BY `admittedon` DESC limit 5");  
 ...
 $adm_pnts->bindParam(':consultantname', $consultantname, PDO::PARAM_STR);
 $adm_pnts->bindValue(':searchname', "%{$searchname}%", PDO::PARAM_STR);
 $adm_pnts -> execute(); 
	}
