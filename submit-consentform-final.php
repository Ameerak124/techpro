...
$response = array();
try{
if(!empty($accesskey) && !empty($umrno) && !empty($doctor_sign) && !empty($doctor_name) && !empty($patient_name) && !empty($witness_name) && !empty($witness_sign) && !empty($patient_sign_one)){
...
$check1 -> execute();
	...
	$stmt1=$pdo4->prepare("INSERT INTO `consent_patient_data`(transid,ipno,`umrno`,billno, `consent_id`, `patient_sign_one`, `patient_name`, `patient_on`, `witness_sign`, `witness_on`, `doctor_sign`, `doctor_name`, `doctor_on`, `interpreter_sign`, `interpreter_on`, `status`,content,`patient_relation`,`witness_relation`,witness_name,interpreter_name,consent_remarks, `created_by`, `created_on`) VALUES (:transid,:ipno,:umrno,:billno,:consent_id,:patient_sign_one,:patientname,CURRENT_TIMESTAMP,:witness_sign,CURRENT_TIMESTAMP,:doctor_sign,:doctor_name,CURRENT_TIMESTAMP,:interpreter_sign,CURRENT_TIMESTAMP,'Active',:content,:patient_relation,:witness_relation,:witness_name,:interpreter_name,:remarks,:userid,CURRENT_TIMESTAMP)");
