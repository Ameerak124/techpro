...
foreach($mmsss as $key) { 
     $percheck ="SELECT `menu`,`submenu` as name,`dashboardtitle` as title,viewtitle,`packagename` as packagename, `classname`, `classtype`,`ioscontrollers` as controller,`storyboardname`,CONCAT(:baseurl1,'icons/',`d-icons`) AS icons,super_ioscontrollers,super_storyboardname,super_packagename FROM `android_permissions_new` WHERE `sno`=:key and source ='Operations'";
     ...
     $temp=[
     "name"=>$rows['name'],
     "viewtitle"=>$rows['viewtitle'],
     "dashboardname"=>$rows['title'],
     "packagename"=>$rows['super_packagename'],
     "classname"=>$rows['classname'],
     "classtype"=>$rows['classtype'],
     "ioscontrollers"=>$rows['super_ioscontrollers'],
     "storyboardname"=>$rows['super_storyboardname'],
     "icon"=>$rows['icons'],
     ];
