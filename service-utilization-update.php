...
$remarks = trim(str_ireplace("'","",$data->remarks));
$ipaddress = $_SERVER['REMOTE_ADDR'];
try {
if(!empty($accesskey) && !empty($track) && !empty($remarks)){
//Check access 
...
if($check -> rowCount() > 0){
// Insert
$saleprice = $pdo4 -> prepare("UPDATE `billing_history` SET `modifiedby` = :userid, `modifiedon` = CURRENT_TIMESTAMP,`remarks` = :remarks,`ipaddress` = :ipaddress,`status` = 'Visible' WHERE `sno` = :track AND `status` = 'Visible' AND `remarks` = ''");
