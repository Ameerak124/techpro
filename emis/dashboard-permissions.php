...
foreach($mmsss as $key) { 
     ...
     $percheck ="SELECT `menu`,`submenu` as name,`dashboardtitle` as title,`super_packagename` as packagename, `classname`, `classtype`,`super_ioscontrollers` as controller,`super_storyboardname` as storyboardname,if(`d-icons`='','',CONCAT(:baseurl,'icons/',`d-icons`)) AS icons,colour FROM `android_permissions_new` WHERE `sno`=:key and source='Emis'";
     ...
     "super_storyboardname"=>$rows['storyboardname'],
     "icon"=>$rows['icons'],
     "colour"=>$rows['colour'],
     ];
