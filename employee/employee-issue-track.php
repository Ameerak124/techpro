...
$ImagePath = "documents/raiseticketattach/$empids";
	...
	$stmt1 = $pdo->prepare("SELECT employee_raise_ticket.sno,(SELECT `status` FROM `employee_issue_logs` WHERE employee_issue_logs.issue_sno=employee_raise_ticket.sno order by `sno` desc LIMIT 1) AS status,employee_raise_ticket.type AS issuetype,employee_raise_ticket.category,employee_raise_ticket.issues,if(employee_raise_ticket.`issues`='Any other Suggestions',employee_raise_ticket.`issue_details`,'') as issue_remarks,CONCAT(':baseurl',`photo`) AS file,employee_raise_ticket.remarks,`assigned_person`,Date_format(`created_on`,'%d-%b-%Y %h:%i %p') AS `createdon`,`user_name`,user_id,`ticket_id`,case when status like '%Pending%'  then  '#F1732E' when status like '%Rejected%'  then  '#E65C12' when status like '%Resolved%'  then  '#43A047' else '#43A047' end  as colour FROM employee_raise_ticket WHERE employee_raise_ticket.ticket_id=:ticket_id");
	...
	$stmt5= $pdo->prepare("SELECT case when status like '%Pending%'  then  '#F1732E' when status like '%Rejected%'  then  '#E65C12' when status like '%Resolved%'  then  '#43A047' else '#43A047' end  as colour,case when `status`like '%Pending%' then concat(:baseurl,'appicons/pending.png') else '' end  as pendingicon FROM employee_issue_logs WHERE ticket_id=:ticket_id  and status=:status");
...
/*  $response['issue_remarks']="issue_remarks"; */
...
$mylist= $pdo->prepare("SELECT `sno`,`issue_sno`,`assigned_person`,RTRIM(`role`) AS designation,`created_by`,Date_format(`created_on`,'%d-%b-%Y %h:%i %p') AS `createdon`,`reason`,`status`,`created_by_name`,   if(`status` like '%Reject%','#FFD9D9',if(`status` like '%Assign%','#E7FDEA',if(`status` like '%Escalate%','#E5E9FF',if(`status` like '%Resolve%','#BBF4DB','#f5b3ae')))) AS backcolor,if(`status` like '%Reject%','#CC0000',if(`status` like '%Assign%','#43A047',if(`status` like '%Escalate%','#43A047',if(`status` like '%Resolve%','#43A047','#E65C12')))) AS textcolor, case when status like '%Pending%'  then  '#F1732E' when status like '%Reject%'  then  '#E65C12' when status like '%Resolve%'  then  '#43A047' else '#43A047' end  as colour,case when `status`like '%Pending%' then concat(:baseurl,'appicons/pending.png') when `status` like '%Reject%' then concat(:baseurl,'appicons/rejected.png') else concat(:baseurl,'appicons/accepted.png') end  as pendingicon ,(select status from employee_issue_logs where employee_issue_logs.ticket_id=:ticket_id order by sno desc limit 1) as realstatuss FROM `employee_issue_logs` where employee_issue_logs.ticket_id=:ticket_id order by sno DESC");
 ...
 if($mylist ->rowCount() > 0){	
	...
	$stmt6= $pdo->prepare("SELECT case  when status like '%Rejected%'  then  '#E65C12' when status like '%Resolved%'  then  '#43A047' else '#43A047' end  as colour,case when `status`=:realstatus then '#F1732E' else   '#43A047' end  as colour,case when `status` like :realstatus and (`status` not like '%Reject%' and `status` not like '%Resolve%')  then concat(:baseurl,'mobile/employee/appicons/pending.png') else  concat(:baseurl,'appicons/accepted.png') end  as pendingicon,if((:status=:realstatus and :status not like '%Reject%' and :status not like '%Resolve%'),concat(:baseurl,'appicons/pending.png'),if((:status=:realstatus and :status like '%Reject%'),concat(:baseurl,'appicons/rejected.png'),concat(:baseurl,'appicons/accepted.png')))  as iconcurr,if((:status=:realstatus and :status not like '%Reject%' and :status not like '%Resolve%'),'#F1732E',if((:status=:realstatus and :status like '%Reject%'),'#CC0000',if((:status=:realstatus and :status like '%Resolve%'),'#43A047', '#43A047')))  as colourcurr FROM employee_issue_logs WHERE ticket_id=:ticket_id  order by sno desc limit 1");
