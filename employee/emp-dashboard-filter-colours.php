...
if(!empty($accesskey)){
$validate = $pdo -> prepare("SELECT `userid`,`branch`,`cost_center`,role FROM `user_logins` WHERE `mobile_accesskey` = :accesskey AND `status` = 'Active'");
$validate->bindParam(':accesskey', $accesskey, PDO::PARAM_STR);
$validate -> execute();
if($validate->rowCount()>0){
		$row = $validate->fetch();
	
	$validate1 = $pdo -> prepare("SELECT if(`assign`='0','','assign') as assign,if(`escalate`='0','','escalate') as escalate,if(`resolve`='0','','resolve') as resolve,if(`reject`='0','','reject') as reject,if(`inprocessing`='0','','inprocessing') as inprocessing FROM `issue_level_designations` where `CL`=:role");
$validate1->bindParam(':role', $row['role'], PDO::PARAM_STR);
$validate1 -> execute();
