...
try{
if(!empty($accesskey) && !empty($type) && !empty($category) && !empty($issues)){
...
$time=date("Y-m-d-H-i-s");		
	if(!empty($raiseticketattachfilename)){ 
		...
		$stmt2= $pdo->prepare("SELECT concat(user_logins.username,' - ','(',user_logins.userid,')') as assigningperson,`tokenid`,username ,role FROM `issue_level_designations` inner join user_logins on user_logins.role=issue_level_designations.`NL`  where cost_center =:branch and issue_level_designations.CL=:role  limit 1");
...
$insert=$pdo->prepare("INSERT INTO `employee_raise_ticket`(`user_id`, `user_name`, `role`, `branch`, `type`, `ticket_id`, `category`, `issues`, `issue_details`, `remarks`, `photo`, `status`, `created_by`, `created_on`,`assigned_person`,realstatus) VALUES (:userid,:user_name,:role,:branch,:type,:ticket_id,:category,:issues,:issue_details,:remarks,:raiseticketattach,'Pending',:userid,CURRENT_TIMESTAMP,:assigningperson,:realstatus)");
