...
$percheck ="SELECT `menu`,`submenu` As name,`dashboardtitle` As title,`packagename` As packagename,`ioscontrollers` as webcontroller,`classname`, `classtype`,`storyboardname`,CONCAT(:baseurl,'appicons/',`d-icons`) as icon  FROM `android_permissions_new` WHERE `sno`=:key AND `source` ='E assist' and status='Active'";
