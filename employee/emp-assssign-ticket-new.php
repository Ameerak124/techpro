...
$design= $person[1];
  $stmt11=$pdo->prepare("SELECT `ticket_id` FROM `employee_raise_ticket` WHERE `sno`=:sno");
   ...
   $rows = $stmt11->fetch(PDO::FETCH_ASSOC);
	...
	$stmt1=$pdo->prepare("UPDATE `employee_raise_ticket` SET assigned_person=:assigning_person,`remarks`=:remarks,realstatus=:statusact,category_type=:categorytype WHERE `sno`=:sno");
	...
	$stmt2=$pdo->prepare("INSERT INTO `employee_issue_logs`( `issue_sno`, `assigned_person`,role, `created_by`, `created_on`, `reason`, `status`,created_by_name,ticket_id,`category_type`) VALUES(:sno,:assigning_person,:designation,:empidd,CURRENT_TIMESTAMP,:remarks,:status,:name,:ticket_id,:categorytype)");
	$stmt2->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt2->bindParam(":categorytype", $categorytype, PDO::PARAM_STR);
...
$design= $person[1];
  $stmt11=$pdo->prepare("SELECT `ticket_id` FROM `employee_raise_ticket` WHERE `sno`=:sno");
   ...
   $rows = $stmt11->fetch(PDO::FETCH_ASSOC);
   	...
   	$statusact='Pending by '.$design;
	$stmt1=$pdo->prepare("UPDATE `employee_raise_ticket` SET assigned_person=:assigning_person,`remarks`=:remarks,realstatus=:statusact WHERE `sno`=:sno");
	$stmt1->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt1 -> bindParam(":assigning_person", $persondata, PDO::PARAM_STR);
	$stmt1->bindParam(":sno", $sno, PDO::PARAM_STR);
	$stmt1->bindParam(":statusact", $statusact, PDO::PARAM_STR);
	...
	$stmt2=$pdo->prepare("INSERT INTO `employee_issue_logs`( `issue_sno`, `assigned_person`,role, `created_by`, `created_on`, `reason`, `status`,created_by_name,ticket_id) VALUES(:sno,:assigning_person,:designation,:empidd,CURRENT_TIMESTAMP,:remarks,:status,:name,:ticket_id)");
	$stmt2->bindParam(":remarks", $remarks, PDO::PARAM_STR);
	$stmt2 -> bindParam(":assigning_person", $persondata, PDO::PARAM_STR);
	$stmt2 -> bindParam(":designation", $designation, PDO::PARAM_STR);
	$stmt2 -> bindParam(":ticket_id", $rows['ticket_id'], PDO::PARAM_STR);
