...
if($check -> rowCount() > 0){
	...
	while($result1 = $stmt->fetch(PDO::FETCH_ASSOC)){
		...
		$center_count = $pdo -> prepare("SELECT COALESCE((SELECT count(*) FROM `employee_raise_ticket` WHERE `realstatus`in ('Pending at Unit Head','Pending by Sub unit head') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate),0) AS unitheadpending,COALESCE((SELECT count(*) FROM `employee_raise_ticket` WHERE `realstatus`IN ('Pending by Corporate Head','Pending by Sub Corporate Head') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate),0) AS pencorhead,COALESCE((SELECT count(*) FROM `employee_raise_ticket` WHERE `realstatus`in ('Pending by Director Operations','Pending by Sub Director Operations') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate),0) AS pendirector,COALESCE((SELECT count(*) FROM `employee_raise_ticket` WHERE `realstatus`IN ('Pending by ED','Pending by Sub ED') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate),0) AS pendinged,COALESCE((SELECT count(*) FROM `employee_raise_ticket` WHERE (`realstatus` LIKE '%Resolve%' or `realstatus` LIKE  '%Reject%') and `branch`=:branch and date(`created_on`) BETWEEN :fdate and :tdate),0) AS closedged");
	...
	$my_array = array("Unit","Unit Head","Corporate Head","Dir&Oper","ED","Closed");
