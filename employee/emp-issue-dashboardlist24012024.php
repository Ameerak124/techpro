...
/* 	$stmt=$pdo->prepare("SELECT `display_name`, `branch_name`, `cost_center` FROM `branch_master` where display_name!='test' group by `cost_center`");
	...
	$revenue_list = $pdo -> prepare("SELECT employee_raise_ticket.`sno`, `type`, employee_raise_ticket.`ticket_id`,user_name, `category`, `issues`,if(employee_raise_ticket.`issues`='Any other Suggestions',employee_raise_ticket.`issue_details`,'') as issue_remarks, `issue_details`, `remarks`, `photo`, case when employee_raise_ticket.`status`='Pending' then concat(:baseurl,'appicons/pending.png') else concat(:baseurl,'appicons/solve.png') end  as icon , CASE WHEN employee_raise_ticket.`status`='Pending' THEN '#F1732E' ELSE '#419505' END AS color ,employee_raise_ticket.`created_by`, date_format(employee_raise_ticket.`created_on`,'%d-%b-%Y') as created_on , employee_raise_ticket.`modified_by`, employee_raise_ticket.`modified_on`, if(`photo`='Please select an image file to upload.' OR `photo`='Yes' or `photo`='Invalid image file'  or `photo`='','',concat(:baseurl,`photo`)) as image ,employee_raise_ticket.status ,assigned_person,realstatus ,employee_raise_ticket.branch,branch_master.display_name as branchname FROM `employee_raise_ticket` inner join branch_master on employee_raise_ticket.branch=branch_master.cost_center WHERE `realstatus`in ('Pending by Director Operations','Pending at Director Operations','Inprocessing by Director Operations') and employee_raise_ticket.`branch`=:branch and date(employee_raise_ticket.`created_on`) BETWEEN :fdate and :tdate ORDER BY  employee_raise_ticket.created_on DESC");
