...
$time=date("Y-m-d-H-i-s");		
	if(!empty($raiseticketattachfilename)){ 
		...
		$rolecheck=$pdo->prepare("SELECT `NL` FROM `employee_level_designations` where `status`='1' and `CL`=:role");
		$rolecheck->bindParam(":role", $result['role'], PDO::PARAM_STR);
		$rolecheck->execute();
		...
		IF($role=='Employee'){
		$stmt2= $pdo->prepare("SELECT concat(user_logins.username,' - ','(',user_logins.userid,')') as assigningperson,`tokenid`,username ,role FROM `employee_level_designations` inner join user_logins on user_logins.role=employee_level_designations.`NL`  where user_logins.cost_center =:branch and employee_level_designations.CL=:role and employee_level_designations.`status`='1'");
		$stmt2->bindParam(":branch", $branch, PDO::PARAM_STR);
		$stmt2->bindParam(":role", $role, PDO::PARAM_STR);
		$stmt2->execute();
		}else if($role=='UNIT HR'){
			$stmt2= $pdo->prepare("SELECT concat(user_logins.username,' - ','(',user_logins.userid,')') as assigningperson,`tokenid`,username ,role FROM `employee_level_designations` inner join user_logins on user_logins.role=employee_level_designations.`NL` where FIND_IN_SET(:branch,user_logins.branch)<>0 and employee_level_designations.CL=:role and employee_level_designations.`status`='1'");
		$stmt2->bindParam(":branch", $branch, PDO::PARAM_STR);
		$stmt2->bindParam(":role", $role, PDO::PARAM_STR);
		$stmt2->execute();
			
		}else{
			$stmt2= $pdo->prepare("SELECT concat(user_logins.username,' - ','(',user_logins.userid,')') as assigningperson,`tokenid`,username ,role FROM `employee_level_designations` inner join user_logins on user_logins.role=employee_level_designations.`NL` where employee_level_designations.CL=:role and employee_level_designations.`status`='1'");
...
if ($insert->rowCount()>0){
   ...
   $response['message']="No Data inserted";
	...
	http_response_code(503);	
   $response['error']=true;
   $response['message']="Please contact Techsupport";
		}
}else{
