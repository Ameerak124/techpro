...
$stmt22= $pdo->prepare("SELECT `assigned_person`,created_by  FROM `employee_issue_logs` WHERE `issue_sno`= :sno and `status`like '%Pending%'");
	   ...
	   $type = $status .  ' by ' .$role;
		...
		if($status=="Escalated"){
	$stmt1=$pdo->prepare("UPDATE `employee_raise_ticket` SET assigned_person=:assignperson,category_type=:categorytype WHERE `sno`=:sno");
	...
	$stmt21=$pdo->prepare("INSERT INTO `employee_issue_logs`(  `issue_sno`, `ticket_id`, `assigned_person`, `role`, `created_by`, `created_by_name`, `created_on`, `reason`, `status`,`category_type`)VALUES (:sno,:ticket_id,:assignperson,:role,:empidd,:name,CURRENT_TIMESTAMP,:remarks,:status,:categorytype)");
	...
	$stmt21->execute();
			...
			$stmt1=$pdo->prepare("UPDATE `employee_raise_ticket` SET assigned_person=:assignperson,status=:status,realstatus=:type,category_type=:categorytype WHERE `sno`=:sno");
